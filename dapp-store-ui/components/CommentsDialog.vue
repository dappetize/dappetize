<template>
  <div>
    <v-card style="max-width: 700px">
      <v-toolbar>
        <img :src="app.mainBanner" class="ml-4" style="border-radius: 20%; width: 50px" />
      ...
      <div style="text-align: center">
        <v-rating v-model="rateValue" hover density="comfortable" color="#01875f" size="x-large"
          :readonly="state.rateStatus.rated"></v-rating>
        <v-text-field variant="solo" style="width: 95%; margin: auto" label="Describe your experience (optional)"
          v-model="commentText"></v-text-field>
      </div>
      <v-btn class="ma-4" @click="postReviewAndRate(),closeModal()">submit</v-btn>
    </v-card>
  </div>
</template>
...
export default {
  ...
  // and context is for emit when i want send data to parant componet via child component
  {
    ...
    const checkUserRating = async (versionId) => {
      userToken.value = await $auth.token();
      try {
    ...
    //events
    onMounted(async () => {
      await checkUserRating(app.version.id)
    })
    return {
      app,
      closeModal,
